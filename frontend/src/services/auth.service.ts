...
export class LoginService {
  static loginWithEmail = async (
    data: emailLoginType,
  ): Promise<SuccessResponse<loginSuccessType>> => {
...
export class RegisterService {
  static registerWithPhone = async (
    data: phoneRegisterType,
  ): Promise<SuccessResponse<registerSuccessData>> => {
...
export class AuthService {
  ...
  static forget_resend = async (
    user_id: string,
  ): Promise<SuccessResponse<null>> => {
    const res = await publicAPI.post(`/auth/forget/resend/${user_id}`);
    return res.data;
  };
  static forget_verify = async (
    data: verifyOtp,
  ): Promise<registerSuccessData> => {
    const res = await publicAPI.post("/auth/forget/verify", data);
    return res.data.data;
  };
  static password_change_after_forget = async (
    data: forget_password_change_schema,
  ): Promise<SuccessResponse<null>> => {
    const res = await publicAPI.post("/auth/forget/change", data);
    return res.data;
  };

  static resend_otp_register = async (
    user_id: string,
  ): Promise<SuccessResponse<null>> => {
    const res = await publicAPI.post("/auth/verify/resend/otp", {
      user_id: user_id,
    });
    return res.data;
  };
