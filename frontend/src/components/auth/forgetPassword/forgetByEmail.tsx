...
export default function ForgotByEmail() {
  ...
  return (
    <div className="space-y-6 pt-5">
        ...
        {open &&
        userID && <VerifyForget open={open} onClose={()=>setOpen(false)} data={{user_id:userID,field_name:"email",field_value:form.getValues("email")}}/>
        }
      {/* Email */}
      <div>
        ...
        {form.formState.errors?.email && (
            <p className="text-xs mt-2 text-red-600">{form.formState.errors?.email?.message}</p>
        )}
      </div>

      {/* Button */}
      <Button onClick={()=>forgetByEmailMutation.mutate({email:form.getValues("email")},{
        onSuccess:(data)=>{
      ...
      })} disabled={forgetByEmailMutation.isPending} className="h-12 w-full rounded-xl bg-orange-500 font-semibold transition hover:bg-orange-600">
