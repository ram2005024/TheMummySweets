...
class ProductRepo:
    ...
    async def create(self,data:ProductCreate):
        categories=(await self.db.execute(select(Category).where(Category.id.in_(data.category_ids)))).scalars().all()
    ...
    async def find_image_by_hash(self,hash_value):
        return (await self.db.execute(select(Image).where(Image.hash_value==hash_value))).scalar_one_or_none()

    async def upload_main_product_image(self,image:str,product_id:UUID):
        product=(await self.db.execute(select(Product).where(Product.id==product_id))).scalar_one()
