...
class UserRegiser(BaseModel):
    ...
    def validate_email_phone(self):
        if self.email and self.mobile_number:
            raise HTTPException(
                status_code=400, detail="Either email or phone number can be registered"
            )
        if not self.email and not self.mobile_number:
            raise HTTPException(
                status_code=400,
                detail="Either email or phone number should be provided",
            )
...
class UserLogin(BaseModel):
    ...
    def validate_email_phone(self):
        if self.email and self.mobile_number:
            raise HTTPException(
                status_code=400,
                detail="Either email or phone number can be used for login",
            )
        if not self.email and not self.mobile_number:
            raise HTTPException(
                status_code=400,
...
class ForgetPassSchema(BaseModel):
    ...
    def check_both_field(self):
        if self.email and self.mobile_number:
            raise HTTPException(
                status_code=400,
                detail="Either email or phone number can be used to verify",
            )
        if not self.email and not self.mobile_number:
            raise HTTPException(
                status_code=400,
